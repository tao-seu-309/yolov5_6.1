...
FILE = Path(__file__).resolve() #当前脚本的绝对路径（含脚本）
ROOT = FILE.parents[0]  # YOLOv5 root directory （不含脚本）
...
# 设置环境变量， LOCAL_RANK表示进程内GPU编号，默认设为-1
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))  # https://pytorch.org/docs/stable/elastic/run.html
RANK = int(os.getenv('RANK', -1))   # 表示进程编号，默认为-1
WORLD_SIZE = int(os.getenv('WORLD_SIZE', 1)) # WORLD_SIZE表示全局的进程数，默认为1
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run(data_dict,
                                            ...
                                            iou_thres=0.65 if is_coco else 0.40,  # best pycocotools results at 0.65
                                            single_cls=single_cls,
                                            dataloader=val_loader,
                                            save_dir=save_dir,
                                            save_json=is_coco,
...
def main(opt, callbacks=Callbacks()):
    ...
    # Train
    if not opt.evolve:
        train(opt.hyp, opt, device, callbacks)  #@pt callback?
